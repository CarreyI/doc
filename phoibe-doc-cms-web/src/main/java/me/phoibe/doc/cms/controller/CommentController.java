...
public class CommentController {
    ...
    public String remove(@RequestParam String idstr,HttpServletRequest request){
        ...
        for(String id : ids) {
            phoibeCommentService.deleteByPrimaryKey(Long.parseLong(id));
        }
        LogUtil.writeLog("删除了id为{"+idstr+"}了评论记录", LogUtil.OPER_TYPE_DEL,"评论管理",CommentController.class,request);
        return JsonUtils.toJson(new Result<>(Code.SUCCESS, ""));
    }
}
